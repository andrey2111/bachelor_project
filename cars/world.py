...
from cars.track import plot_map, generate_obstacles
...
class SimpleCarWorld(World):
    ...
    SPEEDING_PENALTY = 32 * 1e0
    MIN_SPEED = 0.1 * 1e0
    MAX_SPEED = 0.5 * 1e0

    size = (800, 600)

    def __init__(self, num_agents, car_map, obstacles, radii, angles, Physics, agent_class, **physics_pars):
        """
        Инициализирует мир
        ...
        """
        self.physics = Physics(car_map, obstacles, **physics_pars)
        self.map = car_map
        self.obs = obstacles
        self.radii = radii
        self.angles = angles
    ...
    def set_agents(self, agents=1, agent_class=None):
        ...
        self._agent_images = []
        self.collisions = {a: 0 for a in self.agents}
    ...
    def run(self, Physics, steps=None, **physics_pars):
        """
    ...
    def _prepare_visualization(self):
        ...
        screen.fill(white)
        scale = plot_map(self.map, self.obs, screen)
        for state in self.agent_states.values():
            s = pygame.Surface((20, 20))
            s.set_colorkey(white)
            s.fill(white)
            pygame.draw.circle(s,red, (10,10), 10)
            pygame.draw.line(s, white, (10,10), (20,10))
